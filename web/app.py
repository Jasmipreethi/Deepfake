...
try:
    ...
    FEATURE_CONFIG = {
        ...
        'hop_length': 512, 'target_t': 63, 'n_mels': 128,
    }
...
class PretrainedAudioEncoder(nn.Module):
    def __init__(self, feature_dim=256, dropout=0.4, intermediate_dim=512):
        ...
        b.fc = nn.Sequential(nn.Linear(b.fc.in_features, intermediate_dim),
                             nn.ReLU(), nn.Dropout(dropout),
                             nn.Linear(intermediate_dim, feature_dim))
        self.backbone = b
    def forward(self, x):
        return self.backbone(F.interpolate(x, (224,224), mode='bilinear', align_corners=False))
...
class TransformerFusion(nn.Module):
    ...
    def forward(self, vf, af):
        ...
        return {'audio_pred': torch.sigmoid(self.audio_cls(c)),
...
class PretrainedFusion(nn.Module):
    ...
    def forward(self, vf, af):
        ...
        return {'audio_pred': torch.sigmoid(self.audio_cls(f)),
...
def _extract_at(path, start_sec, cfg=FEATURE_CONFIG):
    ...
    video_tensor = torch.FloatTensor((np.array(frames) - mean) / std).permute(0,3,1,2)
    ...
    try:
        ...
        mel = torchaudio.transforms.MelSpectrogram(
            ...
            n_fft=cfg['n_fft'], hop_length=cfg['hop_length'])(waveform)
        audio_tensor = torchaudio.transforms.AmplitudeToDB(top_db=cfg.get('top_db', 80))(mel)
        ...
        ax.imshow(audio_tensor[0].numpy(), aspect='auto', origin='lower',
...
def predict_with_windows(model, path, device, n_windows=3, threshold=0.5):
    ...
    if audio_score < threshold and video_score >= threshold:
        ...
        explanation  = 'Audio-visual mismatch detected — audio and video may not belong together.'
    else:
        triggered_by = 'none'
        explanation  = 'No significant manipulation artifacts detected in audio or video.'
...
def load_model(path, device):
    with _model_lock:
        ...
        fusion = 'transformer' if any('transformer' in k for k in state) else 'pretrained'
...
def save_analysis(rec):
    ...
    con.execute('''INSERT INTO analyses VALUES
        (?,?,?,?,?,?,?,?,?,?,?,?,?)''', (
        rec['id'], rec['filename'], rec['model_path'],
        rec['verdict'], rec['joint_score'], rec['audio_score'],
        rec['video_score'], rec['confidence'], rec['threshold'],
        rec['triggered_by'], rec['explanation'],
...
def generate_pdf(rec):
    ...
    verdict_color = colors.HexColor('#22c55e') if rec['verdict']=='REAL' \
    ...
    # Meta
    meta_style = ParagraphStyle('meta', fontSize=9, textColor=colors.HexColor('#64748b'))
    ...
    # Verdict
    verd_style = ParagraphStyle('verdict', fontSize=28, fontName='Helvetica-Bold',
    ...
    story.append(Paragraph(f"Confidence: {conf_pct}%  |  Threshold: {rec['threshold']}",
                            meta_style))
    ...
    # Explanation
    exp_style = ParagraphStyle('exp', fontSize=11, leading=16,
                               textColor=colors.HexColor('#1e293b'),
                               borderPadding=(8,8,8,8),
                               borderColor=colors.HexColor('#e2e8f0'),
                               borderWidth=1, backColor=colors.HexColor('#f8fafc'))
...
def api_analyse():
    ...
    for f in files:
        ...
        try:
            ...
            if r is None:
                results.append({'filename': f.filename, 'error': 'Could not read video'})
                continue
            aid = str(uuid.uuid4())
            rec = {
                'id':          aid,
