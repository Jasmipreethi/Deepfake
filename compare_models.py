...
def plot_all(results, output_dir, threshold=0.5):
    ...
    ax0.set_xticks(x)
    ax0.set_xticklabels(metric_labels, fontsize=10)
    ax0.set_ylim(0, 1.15)
    ...
    ax_roc.grid(alpha=0.4, color='#e2e8f0')
    ...
    for r, color in zip(results, colors):
        ...
        ax_dist.hist(real_s, bins=bins, alpha=0.4, color='#2ecc71',
                     label=f'{r["name"]} Real' if len(results) == 1 else None)
        ax_dist.hist(fake_s, bins=bins, alpha=0.4, color=color,
                     label=r['name'])
    ax_dist.axvline(threshold, color='#e74c3c', linestyle='--',
    ...
    ax_dist.grid(alpha=0.4, color='#e2e8f0')
    ...
    ax_type.set_xticklabels(type_labels, fontsize=10)
    ...
    for idx, (r, color) in enumerate(zip(results[:3], colors)):
        ...
        for vtype, marker, tc in [
            ('real',            'o', '#2ecc71'),
            ('audio_modified',  's', '#f39c12'),
            ('visual_modified', '^', '#9b59b6'),
            ('both_modified',   'D', '#e74c3c'),
        ]:
