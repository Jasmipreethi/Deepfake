...
#let project(
  ...
  date: datetime.today(),
  abstract: [
    *Background*: The proliferation of deepfake technology - synthetic audio-visual media generated through deep learning - poses threats to digital media integrity across finance, politics, and personal security. Traditional unimodal detection systems struggle with modern multimodal deepfakes that manipulate both audio and video streams simultaneously.

    *Aims*: This dissertation aims to design, implement, and evaluate a multimodal audio-visual deepfake detection system using cross-modal transformer fusion, trained on a speaker-disjoint partition of the AV-Deepfake1M++ dataset.
...
This dissertation operates at the intersection of three established research domains: computer vision (spatial and spatio-temporal feature learning), audio signal processing (spectral analysis and neural speech synthesis detection), and cross-modal learning (attention-based fusion of heterogeneous data streams). The theoretical premise underlying multimodal deepfake detection is that generative models, despite their increasing realism, introduce subtle inconsistencies across the audio and visual modalities that neither stream alone can capture. A face-swapped video may appear visually flawless frame by frame, but its lip-synchronisation with synthesised speech will deviate from natural phoneme-viseme alignment in ways detectable by joint analysis. Similarly, a voice-cloned audio track may pass perceptual scrutiny, but its spectral characteristics will not correlate with the observed facial motion patterns of the speaker. This cross-modal inconsistency hypothesis motivates architectures that allow audio and visual representations to interact during feature learning rather than being concatenated post-hoc @Cai2024 @yi2023audiodeepfakedetectionsurvey. The three-head multi-task architecture adopted in this dissertation - producing independent predictions for audio, video, and joint authenticity - operationalises this framework by quantifying the contribution of each modality separately while capturing their interaction through a joint classification head.
...
All training metrics (loss, AUC per head, learning rate, epoch duration) were logged via Weights & Biases to provide a full audit trail. Checkpoint metadata was used to verify that training histories were reproducible across runs. The 100-video test set was sampled from the validation split using `create_test_data.py`, which performs an independent GroupShuffleSplit on the validation speakers (105 training, 27 validation within that subset). Explicit verification confirmed that none of the 21 test speakers overlapped with the 105 training speakers, preserving the speaker-disjoint guarantee at both the main partition level and the test evaluation level.
...
The primary evaluation metric for each head is AUC (Area Under the ROC Curve), which is calculated independently for each binary output. AUC is threshold-independent and is not adversely affected by the class imbalance present in the test set (25 real, 75 fake). The 3:1 fake-to-real ratio reflects the natural distribution of the validation split, where real videos constitute 26.1% of entries; the test set preserves this proportion to avoid introducing artificial balance that would inflate accuracy estimates. Accuracy at the optimal decision threshold (determined by maximising F1), precision, recall, and F1 score are reported as supporting metrics. A model predicting FAKE for all inputs would achieve 75% accuracy — AUC is therefore the more reliable primary measure, consistent with standard practice in deepfake detection evaluation @Rossler2019 @Cai2024.
...
#figure(
  table(
    ...
    [Cost],
    [Free tier unworkable; paid ~\$50+/month],
    [~\$10–15 per 5-epoch run (student-funded)],
...
#figure(
  table(
    ...
    [24 vCPU / 64.4 GB],
    [\$0.028/hr],
    [Feature extraction, CPU debugging, small-batch experiments],
  ),
...
The primary modelling limitation arises from computational and storage constraints. The AV-Deepfake1M++ dataset is distributed as approximately 1.4 TB of compressed archives; the full training split contains over one million clips requiring storage well beyond the available development environment. Consequently, only the validation split (68,851 usable videos) was used as the sole data source, from which training, validation, and test subsets were created via speaker-disjoint partitioning. This is a fundamental limitation of the data regime, not merely a practical inconvenience: the reported metrics cannot be directly compared to any published benchmark because the training data distribution is substantially narrower than what is used in the systems described in Section 5.4. The model was never exposed to the full diversity of speakers, manipulation techniques, and recording conditions present in the training split, and the test set shares the same data source as the training data (albeit with strictly disjoint speaker identities).
...
With a 100-video test set, a single misclassification shifts category accuracy by 4 percentage points. Wilson 95% confidence intervals are reported here — not to make claims of statistical significance, but to quantify the precision limits of the small test set: overall accuracy Model 3 93.0% [86.3%, 96.6%]; Model 2 66.0% [56.3%, 74.5%]; Model 3 per-type fake categories 100% [86.7%, 100.0%] (n=25 each). The wide intervals on per-category results underscore that these accuracy figures represent indicative trends rather than statistically robust benchmarks.
...
The decision to replace Wav2Vec 2.0 with a ResNet18 on mel-spectrograms was initially reluctant, as Wav2Vec's contextual speech representations were expected to provide superior sensitivity to voice cloning artefacts. In retrospect, the mel-spectrogram approach proved more practical given the corrupted MP4 files in the dataset, and the resulting audio head performance — while weaker than the video head — was sufficient for joint detection where video evidence was strong. The key engineering insight was that robust audio loading (torchaudio's FFmpeg backend) was more impactful than the choice of audio architecture, since librosa-based approaches failed silently on the corrupted files common in the dataset.
...
3. _Training AUC and test-set accuracy were not monotonically related in the tested range._ With n=3 loadable checkpoints from 5-epoch training, Model 2 achieved the highest training AUC (0.994) but the lowest at-threshold test accuracy (66.0%), while Model 3 achieved the reverse. This observation, drawn from a small experiment, should be treated as hypothesis-generating rather than confirmatory: whether the pattern would persist, reverse, or stabilise with longer training is unknown (addressing RQ1).
...
_For practitioners._ When deploying detector models under constrained training budgets, the results from this project suggest that maximum validation AUC does not guarantee optimal threshold-based performance. Model 3 (epoch 3) produced better-calibrated scores with zero false positives compared to later-epoch checkpoints — however, this finding is based on a single training trajectory with a 100-video test set and should be treated as an observation warranting further investigation rather than a deployment guideline. Score calibration, evaluated using reliability diagrams, provides a more complete picture of model readiness than AUC or threshold accuracy alone.
...
#appendix[
  ...
  #figure(
    block(width: 100%)[
      #set text(size: 9pt)
      #table(
        columns: (1.2fr, 1.8fr, 0.5fr, 1.5fr),
